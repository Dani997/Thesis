...
int s= counter_totalizer(global_size, result_counter);

TableResultType *result = (TableResultType*) malloc(sizeof(TableResultType) *s);

cp_from_buffer( clStatus, command_queue, global_size, TableResult_clmem,
					result, result_counter, interval_size);


cout << timer.elapsedMicroseconds() << endl;
	cout << s << endl; ;

 write_file2(s, t1, result, "res.csv");
...
void cp_from_buffer(cl_int clStatus, cl_command_queue command_queue, int global_size, cl_mem TableResult_clmem,	TableResultType *result, int *result_counter, int interval_size)
{

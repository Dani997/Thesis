...
for(int i=0; i < 5; i++)
{
	...
	clStatus = clEnqueueWriteBuffer(command_queue, Table1_clmem, CL_TRUE, 0, j * 65536 * sizeof(Table1Type), t1, 0,	NULL, NULL);
...
for(int i=0; i < 5; i++)
{
	...
	clStatus = clEnqueueReadBuffer(command_queue, TableResult_clmem, CL_TRUE, 0,
		j* 65536 * sizeof(TableResultType), result  , 0,
		NULL, NULL);
		clStatus = clFinish(command_queue);
...
cout << counter_totalizer(global_size, result_counter) ;

 write_file(result_counter ,t1, result, interval_size, global_size, "res.csv");

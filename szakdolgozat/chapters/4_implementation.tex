...
Mivel a tábla méret előre nem ismert ezért a változó helyét a \texttt{malloc} függvénnyel kell lefoglalni. De mivel a lekérdezést külön függvény végzi, és a méretet sem tudjuk előre, ezért a következő megoldást alkalmazom:
Első lépésben létrehozok egy pointert ami később a táblára fog mutatni, most még \texttt{NULL} értékű, és ehhez egy változót ami a méretét tárolja. Az adatokat lekérő függvénynek átadom ezeket, és miután a \texttt{Connector} elvégezte a lekérdezést beállítom a méret változó értékét illetve a tábla memória területét csak ekkor foglalom le, és állítom rá a pointert. Ez úgy lehetséges, hogy a függvénynek egy pointerre mutató pointert adok át és ezáltal kettős indirektség jön létre.
...
A kernel meghatározza saját globális azonosítóját és ebből illetve a kapott intervallum méretből meghatározza, honnan kezdőik az ő része a be-, és kimeneten. Ezután elkezdi az intervallumnyi elem megvizsgálását ügyelve arra, hogy a tábla méretet ne lépje túl. Amennyiben egy elem megfelel a szűrési paramétereknek annak indexét és egyéb lekérdezés függő kiszámolt értékeit a kimeneti tömbbe másolja, majd növeli saját számlálóját. Ez a számláló határozza meg, hogy a hozzátartozó kimeneti részen hányadik helyre írja az értékeket.
\begin{figure}[h!]
\centering
...
\newpage
\SubSection{Az elemek kiolvasása egyszerűen}
A kernelkód működéséből látjuk, hogy több módszert is tudunk alkalmazni az elemek kiolvasására. A találatok számát tartalmazó tömböt biztosan ki kell másolnunk, de kimenetre vonatkozóan választhatunk az alábbi lehetőségekből:

Dönthetünk úgy, hogy kimásoljuk a teljes kimenetet, ekkor annak konzolra való kiírása a következő algoritmus alapján zajlik:
\begin{python}
